...
#ifndef EOSIO_ABI
  #define eosio_assert(a, b) if((!(a))) throw std::runtime_error(b);
#endif
...
auto digest_message_ripemd160(const unsigned char* message, size_t message_len) {
    ...
    eosio_assert (ripemd160::ripemd160_digest(&self, digest.data()), "ripemd failed");
    return digest;
}
...
Key string_to_key(std::string_view s, key_type type, const char (&suffix)[suffix_size]) {
    ...
    eosio_assert (memcmp(ripe_digest.data(), whole.data() + result.data.size(), 4) == 0, "checksum doesn't match");
    return result;
}
...
public_key string_to_public_key(std::string_view s) {
    if (s.size() >= 3 && s.substr(0, 3) == "EOS") {
        ...
        eosio_assert (memcmp(ripe_digest.data(), whole.data() + key.data.size(), 4) == 0, "Key checksum doesn't match");
        return key;
...
private_key string_to_private_key(std::string_view s) {
    if (s.size() >= 7 && s.substr(0, 7) == "PVT_R1_")
        ...
        eosio_assert(0, "unrecognized private key format");
}
...
signature string_to_signature(std::string_view s) {
    if (s.size() >= 7 && s.substr(0, 7) == "SIG_K1_")
        ...
        eosio_assert(0, "unrecognized signature format");
}
